...
/**
 *  Testing create route note and attaching pucture.
 *
 */
class RouteNotesTest extends \Codeception\Test\Unit
{
    use ApiTrait;
    ...
    /**
     *  Tests allowed methods to create route note.
     *
     */
    public function testAllowedMethodsToCreateRouteNote()
    {
        $I = $this->tester;

        // Bad methods
        foreach(['PATCH', 'PUT', 'DELETE', 'GET'] as $method) {
            $I->{'send' . $method}('/routes/1/notes', []);
            $I->seeResponseIsJson();
    ...
    /**
     *  Tests allowed methods to attach signature to route note.
     *
     */
    public function testAllowedMethodsToAttachPictureToRouteNote()
    {
        $I = $this->tester;

        // Bad methods
        foreach(['PATCH', 'PUT', 'DELETE', 'GET'] as $method) {
            $I->{'send' . $method}('/routes/notes/1/image', []);
            $I->seeResponseIsJson();
            $I->seeResponseContainsJson([
                'previous' => ['name' => 'Invalid Route'],
            ]);
        ...
        // Success method
        $I->sendPOST('/routes/notes/1/image', []);
        $I->seeResponseIsJson();
        $I->dontSeeResponseContainsJson([
                'previous' => ['name' => 'Invalid Route'],
        ]);
    ...
    /**
     *  Dev token test to create route note.
     *
     */
    public function testDeveloperTokenToCreateRouteNote()
    {
        // Failed
        $this->checkFailedDevToken('/routes/1/notes', 'POST');

        // Successful
        $this->checkDevToken('/routes/1/notes', 'POST');
    }

    /**
     *  Dev token test to attaching picture to route note.
     *
     */
    public function testDeveloperTokenToAttachPictureToRouteNote()
    {
        // Failed
        $this->checkFailedDevToken('/routes/notes/1/image', 'POST');

        // Successful
        $this->checkDevToken('/routes/notes/1/image', 'POST');
    }

    /**
     *  Test for unauthorized access to create route note.
     *
     */
    public function testUnauthorizedAccessToCreateRouteNote()
    {
        $I = $this->tester;
    ...
    /**
     *  Test for unauthorized access to attaching picture to route note.
     *
     */
    public function testUnauthorizedAccessToAttachPictureToRouteNote()
    {
        $I = $this->tester;
        $I->haveHttpHeader('Dev-Token', $this->_developerToken);
    ...
    /**
     *  Test for authorized access to create route note.
     *
     */
    public function testAuthorizedAccessToCreateRouteNote()
    {
        $I = $this->tester;
    ...
    /**
     *  Test for authorized access to attaching picture to route note.
     *
     */
    public function testAuthorizedAccessToAttachPictureToRouteNote()
    {
        $I = $this->tester;
        $this->authByUsername('driverPortland');
    ...
    /**
     *  Test to get access to another route.
     *
     */
    ...
    /**
     *  Check access to another route's attach signature's image.
     *
     */
    public function testAccessToAnotherAttachPictureToRouteNote()
    {
        $I = $this->tester;
        $this->authByUsername('driverPortland');
        ...
        foreach([6, 7, 8, 9] as $id) {
            $I->sendPOST('/routes/notes/' . $id . '/image');
            $I->seeResponseIsJson();
            $I->seeResponseCodeIs(HttpCode::NOT_FOUND);
            $I->seeResponseContainsJson([
                'name' => 'Not Found'
    ...
    /**
     *  Bad validation attributes to create route note.
     *
     */
    public function testBadValidationToCreateRouteNote()
    {
        $I = $this->tester;
        $values = [
            '',
...
Quia laboriosam distinctio et eos ut sint enim rerum. Non optio assumenda amet ut distinctio. Autem sed nemo consequatur quos adipisci',
        ...
        foreach($values as $value) {
            $I->sendPOST('/routes/1/notes', ['text' => $value]);
            $I->seeResponseIsJson();
    ...
    public function testBadValidationToAttachPictureToRouteNote()
    {
        ...
        $tests = [
            ...
            function() use ($I) {
                ...
                $I->sendPOST('/routes/notes/1/image', null, ['image' => Yii::getAlias($filename)]);
            }, 
    ...
    /**
     *  Successful validation to attaching picture to order note. 
     *
     */
